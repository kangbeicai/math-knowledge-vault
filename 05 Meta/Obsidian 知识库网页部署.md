...
2026-08-13 已完成一次本地完整构建：

- VuePress 成功生成 `218` 个页面；
- 根首页 `index.html` 已生成；
...
当前站点已重新启用 `SlimSearch` 全文搜索。此前导致重复索引 ID 的同页重复标题已清理；同时针对中文知识库增加了中文分词配置，索引端与浏览器查询端使用相同的 `Intl.Segmenter("zh-CN")` 分词，多关键词按 AND 组合，并优先展示标题精确匹配的页面。`aliases` 也作为高权重自定义字段进入索引。

SlimSearch 原生的自动补全更偏向空格分词语言。中文查询经过分词后会在内部形成多个词项，直接使用自动补全容易把内部空格带回输入框，因此当前关闭自动补全，只保留实时搜索结果。
