...
1. 临时内容进入 `00 Inbox/`。
2. 判断属于概念、定理结果、方法、问题、来源、练习、项目或复盘。
3. 使用 `90 Templates/` 中对应模板。
4. 补充 Properties、MOC、前置知识和来源。
5. 检查 Properties、公式分隔符和双向链接后提交。

详细流程见 [[05 Meta/笔记生命周期|笔记生命周期]]。

## 文件命名

- 概念笔记：稳定的中文概念名。
- Result：使用稳定的定理、命题或引理名称。
- Method：使用能表达动作或问题类型的方法名。
- MOC：以 `MOC` 结尾。
- 问题笔记：完整问句。
- 来源笔记：标题加作者、年份或可识别标记。
- 周复盘：`YYYY-Www 周复盘`。
...
- 每篇正式概念笔记至少链接到一个 MOC。
- 真正阻塞定义或核心推导的必要前置写入 `prerequisites`。
- 只用于降低理解成本的背景写入 `helpful_prerequisites`。
- 非依赖联系写入 `related`。
...
- 原文摘录必须标注来源。
- 尽量包含最小例子、反例和掌握检验。
- 可独立复用的定理优先使用 [[90 Templates/定理笔记模板|定理笔记模板]]。
- 可跨问题复用的证明、构造或计算流程优先使用 [[90 Templates/方法笔记模板|方法笔记模板]]。
